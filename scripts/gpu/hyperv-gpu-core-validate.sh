...
require_nouveau_dda_or_gpup_fail_closed() {
    ...
    require_log 'nouveau_dda_device_presence_matrix .*dda_nvidia_present=0 .*vendor_id=0x0 .*device_id=0x0 .*class_code=0x0 .*bar_count=0 .*transport=hyperv_vpci .*gpup_dxg_path=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
        "GPU-P-only Nouveau DDA device-presence matrix stays fail-closed without an assigned NVIDIA function"
    ...
    require_log 'nouveau_kms_acceptance_shape_matrix .*kernel_gate=full_linux_shape .*display_create=0 .*engine_object=0 .*mode_config_ready=0 .*crtcs=0 .*encoders=0 .*primary_planes=0 .*vblank_source=none .*page_flip_event_source=none .*linear_required=1 .*nonlinear_modifiers=0 .*native_display_ready=0 .*dda_native_display_present=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
        "GPU-P-only Nouveau KMS native-present gate requires the full Linux display shape"
    require_log 'nouveau_dda_display_positive_shape_matrix .*dda_positive=0 .*display_create=0 .*engine_object=0 .*mode_config_ready=0 .*crtcs=0 .*encoders=0 .*primary_planes=0 .*vblank_source=none .*page_flip_event_source=none .*linear_required=1 .*nonlinear_modifiers=0 .*d3d12_native_present_credit=0 .*opengl_submit_credit=0 .*webkit_credit=0 .*status=PASS_FAILCLOSED' \
        "GPU-P-only Nouveau DDA positive display shape stays fail-closed and separate from D3D12"
...
log_host_gpu_assignment() {
    {
        ...
        powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \
            "\$adapter = @(Get-VMGpuPartitionAdapter -VMName '${VM_NAME}' -ErrorAction SilentlyContinue); \$partitionable = @(Get-VMHostPartitionableGpu -ErrorAction SilentlyContinue); \$dda = @(Get-VMAssignableDevice -VMName '${VM_NAME}' -ErrorAction SilentlyContinue); \$a0 = \$adapter | Select-Object -First 1; \$instance = if (\$a0) { [string]\$a0.InstancePath } else { '' }; \$gpup = if (\$instance -like '*\\GPUPARAV*') { 1 } else { 0 }; \$vf = if (\$a0) { [string]\$a0.PartitionVfLuid } else { 'none' }; \$vram = if (\$a0) { [uint64]\$a0.CurrentPartitionVRAM } else { 0 }; \$compute = if (\$a0) { [uint64]\$a0.CurrentPartitionCompute } else { 0 }; \$status = if (\$adapter.Count -gt 0 -and \$gpup -eq 1) { 'PASS' } else { 'FAIL' }; Write-Output ('host_gpup_partition_contract_matrix vm=${VM_NAME} adapter_present=' + [int](\$adapter.Count -gt 0) + ' adapter_count=' + \$adapter.Count + ' instance_gpup=' + \$gpup + ' partition_vf_luid=' + \$vf + ' partition_vram=' + \$vram + ' partition_compute=' + \$compute + ' partitionable_gpus=' + \$partitionable.Count + ' dda_assignable=' + \$dda.Count + ' selected_transport=gpup custom_host_tool=0 guest_display_bind_sender_contract=0 guest_display_bind_completion_contract=0 native_present_credit=0 opengl_submit_credit=0 status=' + \$status)"
    } >>"${LOG}" 2>&1 || true
}
...
require_log 'dxg_hwqueue_last=.*submit_len:[1-9][0-9]* submit_ret:0 submit_status:0x0' \
    "kernel records raw SUBMITCOMMANDTOHWQUEUE host NTSTATUS"
require_log 'wsl_trace_replay_host_saw_matrix .*make_len:24 .*make_device:0x0 .*make_count:1 .*make_flags:0x1 .*make_sorted:0 .*make_in:[1-9a-fA-F][0-9a-fA-F]*,0 .*make_wire:[1-9a-fA-F][0-9a-fA-F]*,0 .*context_len:[1-9][0-9]* .*context_ret:0 .*hwqueue_len:[1-9][0-9]* .*hwqueue_ret:0 .*submit_queue:0x[1-9a-fA-F][0-9a-fA-F]* .*submit_cmd_len:4096 .*submit_priv:[1-9][0-9]* .*submit_len:[1-9][0-9]* .*submit_ret:0 .*submit_status:0x0 .*host_saw=cat_/dev/dxg_after_replay .*status=PASS' \
    "WSL replay kernel host-saw packet-shape diagnostics"
require_log 'wsl_submit_ntstatus_not_completion_matrix .*submit_hwqueue_cmd=52 .*submit_rc=0 .*submit_ntstatus=0x0 .*submit_success=1 .*vm_pkt_comp_is_d3dkmt_return=1 .*submit_ntstatus_is_display_completion=0 .*submit_success_is_display_bind=0 .*sender_contracts=0 .*completion_contracts=0 .*host_saw_display_bind_packet=0 .*completion_demux_registered=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "successful WSL HWQUEUE submit NTSTATUS is not display completion credit"
...
require_log 'host_display_bind_source_catalog_matrix .*selected_source=missing .*selected_lane=gpup_dxg_scanout_bind .*provider_state=failclosed .*custom_host_tool=0 .*wsl_dxg_display_bind_ioctl=0 .*wslg_channel=absent .*gpup_dxg_sender=0 .*gpup_dxg_completion=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*wsl_presenthistory_completion_credit=0 .*synthvid_d3d12_bind=0 .*dda_d3d12_resource_import=0 .*dda_scanout_bind=0 .*dda_hw_flip_completion=ABSENT .*transport_present=0 .*present_id=0 completed=0 .*provider_no_host_abi=1 .*provider_no_sender=1 .*provider_no_completion=1 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "host display-bind source catalog remains fail-closed without WSLg/GPU-P/DDA sender"
require_log 'host_gpup_partition_contract_matrix .*adapter_present=1 .*instance_gpup=1 .*partition_vf_luid=[^ ]+ .*partition_vram=[1-9][0-9]* .*partitionable_gpus=[1-9][0-9]* .*selected_transport=gpup .*custom_host_tool=0 .*guest_display_bind_sender_contract=0 .*guest_display_bind_completion_contract=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "host Hyper-V GPU-P assignment is present but exposes no display-bind sender/completion contract"
require_log 'd3d12_display_bind_host_abi_discovery_matrix .*custom_host_tool=0 .*wsl_dxg_display_bind_ioctl=0 .*wsl_display_bind_ioctl_absent=1 .*wslg_frame_path=absent .*freerdp_frame_path=absent .*rdp_frame_path=copy_or_dirty_frame .*hvsock_display_bind_service=absent .*gpup_dxg_sender_contract=0 .*gpup_dxg_completion_contract=0 .*completion_demux_contract=0 .*wsl_presenthistory_completion_credit=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*dda_nouveau_d3d12_import=0 .*dda_nouveau_scanout_bind=0 .*dda_nouveau_hw_flip_completion=ABSENT .*provider_state=failclosed .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "bounded D3D12 display-bind host ABI discovery remains source-audited and zero-credit"
require_log 'd3d12_completion_source_authority_matrix .*accepted_completion_source=display_bind_provider .*submit_ntstatus_as_completion=0 .*presenthistory_telemetry_as_completion=0 .*syncfile_fence_as_display_completion=0 .*callback_release_as_display_completion=0 .*kms_vblank_as_d3d12_completion=0 .*provider_no_completion=1 .*provider_completion_present=0 .*completion_contracts=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "D3D12 completion source authority remains display-bind-provider only"
require_log 'native_present_completion_source_namespace_matrix .*completion_source=missing .*d3d12_display_bind_provider=failclosed .*d3d12_present_id=0 d3d12_completed=0 .*kms_vblank_sequence=0 .*kms_page_flip_sequence=0 .*nouveau_irq_cause_valid=0 .*nouveau_irq_cause_acks=0 .*namespace_mixed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "native-present completion namespaces stay separated while fail-closed"
...
require_log 'wsl_dxg_ioctl_namespace_probe_matrix .*last_known_ioctl_nr=0x49 .*display_bind_probe_nr=0x4a .*future_high_nr=0x7f .*wrong_type_rc=-[0-9][0-9]* .*wrong_size_rc=-[0-9][0-9]* .*wrong_dir_rc=-[0-9][0-9]* .*after_last_rc=-[0-9][0-9]* .*future_high_rc=-[0-9][0-9]* .*accepted_ioctls=0 .*display_bind_ioctl_present=0 .*present_source_ioctl_present=0 .*present_completion_ioctl_present=0 .*linux_ioctl_contracts=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "malformed and beyond-WSL DXG ioctls cannot become display-bind contracts"
require_log 'dxg_host_to_vm_presenthistory_completion_matrix .*propagate_presenthistory_cmd=1 .*presenthistory_packets=[0-9][0-9]* .*presenthistory_head_len=[0-9][0-9]* .*completion_contracts=0 .*completion_successes_delta=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
...
require_log 'dxg_presenthistory_orphan_completion_rejection_matrix .*propagate_presenthistory_cmd=1 .*provider_pending_match=0 .*completion_demux_registered=0 .*display_bind_present_id=0 .*display_bind_completed_id=0 .*orphan_completion_rejected=1 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL present-history packets are rejected as orphan display completions"
require_log 'wsl_submit_present_fields_not_bind_matrix .*submit_present_redirected_field_known=1 .*submit_present_history_token_field_known=1 .*written_primaries_field_known=1 .*resource_scanout_bind_sender=0 .*display_completion_return=0 .*sender_contracts=0 .*completion_contracts=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL submit present fields are metadata, not a resource scanout-bind ABI"
require_log 'wsl_submit_ntstatus_not_completion_matrix .*submit_hwqueue_cmd=52 .*submit_ntstatus_is_display_completion=0 .*submit_success_is_display_bind=0 .*sender_contracts=0 .*completion_contracts=0 .*host_saw_display_bind_packet=0 .*completion_demux_registered=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL submit success remains outside the display-bind completion namespace"
require_log 'wsl_stdalloc_and_alloc_flags_not_bind_matrix .*stdalloc_private_data_sender_present=1 .*stdalloc_display_bind_sender=0 .*primary_alloc_flag_known=1 .*direct_flip_alloc_flag_known=1 .*alloc_flags_are_completion_contract=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL standard allocation and allocation flags are not display-bind completion proof"
require_log 'wsl_trace_display_bind_negative_matrix .*trace_paths_checked=2 .*trace_ioctl_namespace_subset_of_wsl_uapi=1 .*display_bind_opcode_seen=0 .*nonzero_present_history_token_seen=0 .*open_resource_without_display_bind_seen=1 .*syncfile_without_display_completion_seen=1 .*resource_share_without_scanout_bind_seen=1 .*linux_ioctl_contracts=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL traces prove resource sharing and sync-file plumbing without display bind"
require_log 'public_present_api_not_guest_bind_matrix .*reactos_d3dkmt_present_api=known .*reactos_present_redirected_api=known .*directx_shared_handle_api=known .*directx_sharing_contract_hwnd_only=1 .*wslg_local_source=absent .*freerdp_local_source=absent .*rdp_frame_transport=copy_or_dirty_frame .*guest_vmbus_display_bind_contract=0 .*gpup_sender_contract=0 .*gpup_completion_contract=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "public Windows/WSLg/RDP present APIs are not a guest display-bind protocol"
require_log 'd3d12_display_bind_host_abi_discovery_matrix .*custom_host_tool=0 .*wsl_dxg_display_bind_ioctl=0 .*wsl_display_bind_ioctl_absent=1 .*wslg_frame_path=absent .*freerdp_frame_path=absent .*rdp_frame_path=copy_or_dirty_frame .*hvsock_display_bind_service=absent .*gpup_dxg_sender_contract=0 .*gpup_dxg_completion_contract=0 .*completion_demux_contract=0 .*wsl_presenthistory_completion_credit=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*dda_nouveau_d3d12_import=0 .*dda_nouveau_scanout_bind=0 .*dda_nouveau_hw_flip_completion=ABSENT .*provider_state=failclosed .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "D3D12 display-bind host ABI discovery remains closed in final backend pass"
require_log '^gpu_core_c_validator d3d12_negative_abi_manifest_matrix .*source_audited=1 .*wsl_uapi_header=d3dkmthk\.h .*wsl_display_bind_ioctl=0 .*wsl_vmbus_file=dxgvmbus\.c .*presenthistory_cmd=34 .*redirected_flip_fence_cmd=35 .*blt_cmd=38 .*submit_hwqueue_cmd=52 .*propagate_presenthistory_cmd=1 .*presenthistory_is_telemetry=1 .*hvsock_display_bind_service=absent .*resource_scanout_bind_sender=0 .*display_completion_demux=0 .*synthvid_path=gpa_dirty_rect_only .*dda_nouveau_path=separate_pci_display .*dda_d3d12_import=0 .*dda_scanout_bind=0 .*dda_hw_flip_completion=ABSENT .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "source-audited negative WSL/Linux ABI manifest remains zero-credit"
require_log '^gpu_core_c_validator d3d12_display_bind_authority_chain_matrix .*chain_version=1 .*selected_lane=gpup_dxg_scanout_bind .*host_abi_gate=closed .*provider_send_gate=closed .*host_packet_gate=closed .*completion_demux_gate=closed .*display_completion_gate=closed .*source_generation_gate=armed .*resource_generation_gate=armed .*consumer_credit_gate=closed .*transport_pending_id=0 .*command_id=0 .*transaction_id=0 .*channel=0 .*display_bind_transport_source=none .*host_saw_display_bind_packet=0 .*completion_demux_registered=0 .*wsl_presenthistory_completion_credit=0 .*dda_native_display_is_d3d12_bridge=0 .*kms_completion_is_d3d12=0 .*syncfile_is_display_completion=0 .*present_id=0 completed=0 .*native_present_credit=0 .*backend_opengl_submit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "D3D12 display-bind authority chain stays closed until every upstream gate is real"
require_log 'provider_credit_gate_negative_matrix .*host_abi_present=0 .*sender_present=0 .*completion_present=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*backend_opengl_submit=0 .*status=PASS' \
    "display-bind provider credit remains closed without host ABI, sender, and completion"
...
require_log 'dda_nouveau_d3d12_bridge_disjoint_matrix .*dda_backend_flag=[01] .*dda_pci_display_present=[0-9]+ .*dda_d3d12_import_path=0 .*dda_d3d12_scanout_bind=0 .*dda_hw_flip_completion_for_d3d12=0 .*kms_lane=[0-9]+ .*kms_lane_is_d3d12=0 .*display_bind_present_id=0 .*display_bind_completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "DDA/Nouveau native-display diagnostics are disjoint from the D3D12 bridge"
...
require_log 'd3d12_completion_source_authority_matrix .*accepted_completion_source=display_bind_provider .*submit_ntstatus_as_completion=0 .*presenthistory_telemetry_as_completion=0 .*syncfile_fence_as_display_completion=0 .*callback_release_as_display_completion=0 .*kms_vblank_as_d3d12_completion=0 .*provider_no_completion=1 .*provider_completion_present=0 .*completion_contracts=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "D3D12 completion source authority remains closed in final backend pass"
require_log 'native_present_completion_source_namespace_matrix .*completion_source=missing .*d3d12_display_bind_provider=failclosed .*d3d12_present_id=0 d3d12_completed=0 .*kms_vblank_sequence=0 .*kms_page_flip_sequence=0 .*nouveau_irq_cause_valid=0 .*nouveau_irq_cause_acks=0 .*namespace_mixed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "native-present completion namespace remains unmixed in final backend pass"
...
require_log 'gpu_remaining_plan_dependency_skeleton_matrix .*root_display_bind_gate=closed .*native_present_gate=closed .*native_completion_validators=armed .*finite_480p_gate=closed .*demo_interaction_gate=closed .*backend_opengl_submit_gate=closed .*kvm_virgl_recheck_gate=deferred .*webkit_route_gate=closed .*webkit_content_gate=closed .*webkit_enabled_artifact_gate=closed .*wsl_display_bind_ioctl=0 .*wsl_inband_presenthistory_handler=absent .*gpup_sender_contract=0 .*gpup_completion_contract=0 .*dda_d3d12_resource_import=0 .*dda_scanout_bind=0 .*dda_hw_flip_completion=ABSENT .*display_bind_present_id=0 .*display_bind_completed=0 .*backend_opengl_submit=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "whole remaining GPU plan dependency skeleton stays closed until native display-bind opens"
require_log 'gpu_remaining_holistic_skeleton_matrix .*skeleton_version=2 .*active_open_items=12 .*ordered_chunks=display_bind,native_completion,fps,backend,webkit .*display_bind_source_gate=closed .*bind_contract_gate=failclosed .*native_completion_gate=armed_without_lane .*finite_480p_gate=closed .*demo_interaction_gate=closed .*backend_opengl_submit_gate=closed .*kvm_virgl_recheck_gate=deferred .*webkit_route_gate=closed .*webkit_content_gate=closed .*webkit_enabled_artifact_gate=closed .*selected_lane=gpup_dxg_scanout_bind .*completion_authority=display_bind_provider .*wsl_display_bind_ioctl=0 .*gpup_sender_contract=0 .*gpup_completion_contract=0 .*dda_d3d12_resource_import=0 .*dda_scanout_bind=0 .*dda_hw_flip_completion=ABSENT .*display_bind_transport_present=0 .*display_bind_present_id=0 display_bind_completed_id=0 .*native_present_credit=0 .*backend_opengl_submit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "holistic remaining GPU skeleton is explicit and fail-closed"
...
require_log 'd3d12_display_bind_pending_lifetime_matrix .*created_delta=[1-9][0-9]* .*active=0 .*completed_delta=0 .*failclosed_delta=[1-9][0-9]* .*last_status=95 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "D3D12 display-bind pending request lifetime resolves fail-closed without native credit"
require_log 'd3d12_display_bind_generation_revalidation_matrix .*provider_submits_delta=[1-9][0-9]* .*lock_dropped_submits_delta=[1-9][0-9]* .*revalidate_attempts_delta=[1-9][0-9]* .*revalidate_successes_delta=[1-9][0-9]* .*revalidate_failures_delta=0 .*provider_pin_revalidated=1 .*source_generation_match=PASS .*resource_generation_match=PASS .*pinned_generation_match=PASS .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "D3D12 display-bind source and resource generations are revalidated after the provider lock drop"
require_log 'd3d12_display_bind_provider_pending_publication_matrix .*provider_submits_delta=[1-9][0-9]* .*publication_attempts_delta=[1-9][0-9]* .*dxgprocess_generation=[1-9][0-9]* .*process_adapter_generation=[1-9][0-9]* .*process_namespace_valid=1 .*hmgr_index_unique_valid=1 .*device_hmgr_index_unique_valid=1 .*resource_hmgr_index_unique_valid=1 .*allocation_hmgr_index_unique_valid=1 .*device_object_ref_active=1 .*resource_object_ref_active=1 .*allocation_object_ref_active=1 .*shared_parent_id=[1-9][0-9]* .*shared_parent_refs=[1-9][0-9]* .*shared_parent_children=[1-9][0-9]* .*shared_parent_snapshot_valid=1 .*opened_child_snapshot_valid=1 .*shared_parent_global_share_match=1 .*syncobject_object_ref_active=[01] .*syncobject_shared_owner_present=[01] .*syncobject_monitored_fence=[01] .*syncobject_fence_value=[0-9][0-9]* .*syncobject_fence_cpu_va_present=[01] .*syncobject_fence_gpu_va_present=[01] .*syncobject_fence_map_size=[0-9][0-9]* .*owner_close_cancelled=0 .*publish_before_send=0 .*transport_pending_id=0 .*completion_demux_registered=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*wsl_presenthistory_completion_credit=0 .*resolved_or_cancelled=1 .*refs_released=1 .*no_host_abi_cancelled=1 .*no_host_abi_refs_released=1 .*provider_no_host_abi=1 .*provider_no_sender=1 .*provider_no_completion=1 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider pending publication carries WSL-shaped process, HMGR, shared-parent, and sync-fence proof while fail-closed"
require_log 'd3d12_display_bind_provider_shared_parent_retention_matrix .*shared_parent_id=[1-9][0-9]* .*shared_parent_refs=[1-9][0-9]* .*shared_parent_fd_refs=[1-9][0-9]* .*shared_parent_host_nt_refs=[1-9][0-9]* .*shared_parent_child_refs=[1-9][0-9]* .*shared_parent_children=[1-9][0-9]* .*shared_parent_global_share=[1-9][0-9]* .*shared_parent_host_nt=[1-9][0-9]* .*opened_child_parent_id_match=1 .*opened_child_global_share_match=1 .*opened_child_sealed_generation_match=1 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider retains WSL-shaped shared-resource parent state"
require_log 'd3d12_display_bind_provider_sync_fence_alias_matrix .*syncobject_object_ref_active=[01] .*syncobject_fence_cpu_va_present=[01] .*syncobject_fence_kva_present=[01] .*syncobject_fence_gpu_va_alias_gap=[01] .*real_fence_gpu_va_present=[01] .*gpu_va_source=[0-9][0-9]* .*kva_is_real_gpu_va=0 .*syncfile_dma_fence_display_completion_credit=0 .*host_saw_display_bind_packet=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider names sync fence KVA alias gap without display credit"
require_log 'gpu_core_c_validator d3d12_display_bind_provider_packet_lifetime_matrix .*packet_listed=0 .*request_id=0 .*transport_pending_id=0 .*command_id=0 .*transaction_id=0 .*channel=0 .*packet_completed=0 .*wait_cancelled=0 .*packet_removed_on_cancel=0 .*completion_demux_registered=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS_FAILCLOSED' \
    "pure-C provider packet lifetime remains empty and zero-credit until a real sender publishes a packet"
require_log 'd3d12_display_bind_provider_no_send_preflight_matrix .*request_metadata_complete=1 .*provider_pin_revalidated=1 .*source_generation_match=PASS .*resource_generation_match=PASS .*preflight_ready=1 .*send_attempts=0 .*send_blocked_no_host_abi=[1-9][0-9]* .*completion_demux_attempts=0 .*completion_demux_blocked_no_contract=[1-9][0-9]* .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*provider_no_host_abi=1 .*provider_no_sender=1 .*provider_no_completion=1 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider reaches ready-to-send preflight but records no host ABI, sender, or completion contract"
...
require_log 'd3d12_native_completion_consumer_escrow_matrix .*display_bind_gate=closed .*provider_completion_present=0 .*transport_present=0 .*completion_demux_registered=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*present_id=0 completed=0 .*callback_release_credit=0 .*frame_callback_credit=0 .*final_handoff_credit=0 .*fps_visible_credit=0 .*content_progress_credit=0 .*webkit_accel_credit=0 .*callback_release_order=blocked .*consumer_visible_credit=blocked .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "native D3D12 completion consumers remain escrowed until display-bind completion"
require_log 'd3d12_display_bind_stale_source_zero_credit_matrix .*release_sources_delta=[0-9]+ .*after_close_queries=[1-9][0-9]* .*stale_source_rejects=[1-9][0-9]* .*release_clears=[0-9]+ .*late_completion_after_release=0 .*after_close_present_id=0 .*after_close_completed=0 .*global_present_id_after_close=0 .*global_completed_after_close=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*cleanup_state=(immediate|deferred_until_process_exit) .*status=PASS' \
    "stale display-bind source evidence remains zero-credit after owner close"
require_log 'gpu_core_c_validator d3d12_display_bind_stale_source_zero_credit_matrix .*after_close_queries=[1-9][0-9]* .*stale_source_rejects=[1-9][0-9]* .*release_clears=[1-9][0-9]* .*late_completion_after_release=0 .*global_present_id_after_close=0 .*global_completed_after_close=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
    "parent C validator observes deferred display-bind source cleanup"
require_log 'gpu_core_c_validator d3d12_display_bind_stale_async_completion_contract_matrix .*real_sender=0 .*sender_state=absent .*async_completion_path=absent .*stale_async_scope=no_sender_failclosed .*completion_demux_registered=0 .*transport_pending_id=0 .*owner_close_cancel_required_after_send=1 .*owner_close_cancel_required_after_real_send=1 .*owner_close_cancel_after_send_tested=0 .*owner_close_cancel_deferred=1 .*owner_close_cancel_deferred_until_sender=1 .*late_completion_reject_required=1 .*late_completion_reject_required_after_real_send=1 .*late_completion_reject_gate=armed .*late_completion_reject_tested=0 .*late_completion_reject_deferred_until_sender=1 .*late_completion_after_release=0 .*late_completion_rejected=not_sampled .*late_completion_reject_observed=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS_FAILCLOSED' \
    "parent C validator keeps stale async display-bind completion fail-closed without a real sender"
...
require_log 'wsl_submit_present_fields_not_bind_matrix .*submit_present_redirected_field_known=1 .*submit_present_history_token_field_known=1 .*written_primaries_field_known=1 .*resource_scanout_bind_sender=0 .*display_completion_return=0 .*sender_contracts=0 .*completion_contracts=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL submit present fields remain zero-credit in final backend pass"
require_log 'wsl_submit_ntstatus_not_completion_matrix .*submit_hwqueue_cmd=52 .*submit_ntstatus_is_display_completion=0 .*submit_success_is_display_bind=0 .*sender_contracts=0 .*completion_contracts=0 .*host_saw_display_bind_packet=0 .*completion_demux_registered=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL submit NTSTATUS remains zero-credit in final backend pass"
require_log 'wsl_stdalloc_and_alloc_flags_not_bind_matrix .*stdalloc_private_data_sender_present=1 .*stdalloc_display_bind_sender=0 .*primary_alloc_flag_known=1 .*direct_flip_alloc_flag_known=1 .*alloc_flags_are_completion_contract=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL standard allocation and allocation flags remain zero-credit in final backend pass"
require_log 'wsl_trace_display_bind_negative_matrix .*trace_paths_checked=2 .*trace_ioctl_namespace_subset_of_wsl_uapi=1 .*display_bind_opcode_seen=0 .*nonzero_present_history_token_seen=0 .*open_resource_without_display_bind_seen=1 .*syncfile_without_display_completion_seen=1 .*resource_share_without_scanout_bind_seen=1 .*linux_ioctl_contracts=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL trace replay evidence stays negative in final backend pass"
require_log 'provider_credit_gate_negative_matrix .*host_abi_present=0 .*sender_present=0 .*completion_present=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*backend_opengl_submit=0 .*status=PASS' \
    "provider credit gate stays closed in final backend pass"
require_log 'dda_nouveau_non_readback_display_proof_matrix .*dda_pci_transport_present=GPU_P_FAIL_CLOSED .*dda_nouveau_display_present=ABSENT .*dda_nouveau_non_readback_present=ABSENT .*display_create_successes=0 .*heads=0 .*connectors=0 .*nonvirtual_connector=0 .*vblank_supported=0 .*vblank_irqs=0 .*page_flip_completions=0 .*kms_lane=0 .*kms_present_dumb=0 .*kms_present_synthvid=0 .*kms_present_nouveau_hw=0 .*kms_vblank_source_nouveau_hw=0 .*page_flip_events_native_hw=0 .*dda_native_display_credit=0 .*d3d12_native_present_credit=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
