...
run_policy_negative_preflight()
{
    ...
    printf '%s\n' \
        ...
        'webkit_gpu_policy requested_accel=1 effective_accel=1 gpu_contract=d3d12-shared-surface app_loop_only=1 d3d12_present=0 fallback=none' \
        ...
        'webkit_gpu_policy requested_accel=1 effective_accel=1 gpu_contract=d3d12-shared-surface backend_opengl_submit=0 display_bind_present_id=9 display_bind_completed_id=9 native_present_id=9 native_completed=9 fallback=none' \
        'webkit_gpu_policy requested_accel=1 effective_accel=1 gpu_contract=d3d12-shared-surface validation_run_id=current d3d12_run_id=stale display_bind_present_id=9 display_bind_completed_id=9 backend_opengl_submit=1 fallback=none' \
        'webkit_gpu_policy requested_accel=1 effective_accel=1 gpu_contract=d3d12-shared-surface validation_run_id=current fps_artifact_run_id=stale backend_opengl_submit=1 display_bind_present_id=9 display_bind_completed_id=9 fallback=none' \
        >"${tmp}"
    ...
    grep -Eq 'effective_accel=1 .*app_loop_only=1' "${tmp}" ||
        fail "policy preflight failed to reject app-loop-only evidence"
    grep -Eq 'effective_accel=1 .*software_fallback=1' "${tmp}" ||
        fail "policy preflight failed to reject software-fallback evidence"
    grep -Eq 'effective_accel=1 .*backend_opengl_submit=0 .*display_bind_present_id=[1-9]' "${tmp}" ||
        fail "policy preflight failed to reject nonzero ids with backend_opengl_submit=0"
    grep -Eq 'effective_accel=1 .*validation_run_id=current .*d3d12_run_id=stale' "${tmp}" ||
        fail "policy preflight failed to reject stale D3D12 lineage"
    grep -Eq 'effective_accel=1 .*validation_run_id=current .*fps_artifact_run_id=stale' "${tmp}" ||
        fail "policy preflight failed to reject stale FPS artifact lineage"
    rm -f "${tmp}"

    echo "hyperv-webkit-gpu-validate: webkit_evidence_rejection_matrix title_only=PASS chrome_only=PASS cursor_only=PASS callback_only=PASS release_only=PASS render_node_only=PASS dmabuf_only=PASS app_loop_only=PASS env_only=PASS software_fallback=PASS backend_zero_with_ids=PASS d3d12_lineage_mismatch=PASS fps_lineage_mismatch=PASS status=PASS" |
        tee -a "${LOG}"
    echo "hyperv-webkit-gpu-validate: webkit_display_bind_dependency_skeleton_matrix validation_run_id=${VALIDATION_RUN_ID} evidence_path=/tmp/wlcomp-d3d12-present required_current_run_display_bind_completion=1 required_validated_shared_surface_native_d3d12_contract=1 required_content_progress_native_present_complete=1 required_backend_opengl_submit=1 title_only=REJECT chrome_only=REJECT cursor_only=REJECT render_node_only=REJECT dmabuf_only=REJECT app_loop_only=REJECT stale_log_rejected=PASS display_bind_backend=none display_bind_transport=none display_bind_transport_source=none host_saw_display_bind_packet=0 wsl_presenthistory_completion_credit=0 display_bind_present_id=0 display_bind_completed_id=0 display_bind_resource_generation=0 display_bind_completion_source=missing completion_source=missing current_run_display_bind_completion=0 validated_shared_surface_native_d3d12_contract=0 content_progress_native_present_complete=0 backend_opengl_submit=0 gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_content_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
    echo "hyperv-webkit-gpu-validate: webkit_animated_content_native_present_gate_matrix fixture=${WEBKIT_GPU_ANIMATED_CONTENT_FIXTURE} required_compositor_owned_visible_content=1 required_content_crc_progress=1 required_frame_hash_progress=1 required_current_webkit_run_id=1 required_same_client_resource_generation_identity=1 required_callback_release_ordering=1 required_prior_fps_native_present_contract=1 required_backend_opengl_submit=1 required_native_present_completion=1 title_only=REJECT chrome_only=REJECT cursor_only=REJECT env_only=REJECT render_node_only=REJECT dmabuf_only=REJECT compositor_owned_visible_content=0 content_crc_progress=0 frame_hash_progress=0 current_webkit_run_id=0 same_client_resource_generation_identity=0 callback_release_ordering=0 prior_fps_native_present_contract=0 backend_opengl_submit=0 native_present_completion=0 gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
    echo "hyperv-webkit-gpu-validate: webkit_enabled_artifact_contract_matrix required_current_run_display_bind_completion=1 required_finite_480p_fps_artifact=1 required_backend_opengl_submit=1 required_shared_resource_fence_identity=1 required_compositor_owned_content_crc_frame_hash_identity=1 current_run_display_bind_completion=0 finite_480p_fps_artifact=0 backend_opengl_submit=0 shared_resource_fence_identity=0 compositor_owned_content_crc_frame_hash_identity=0 display_bind_transport_source=none host_saw_display_bind_packet=0 wsl_presenthistory_completion_credit=0 display_bind_present_id=0 display_bind_completed_id=0 display_bind_resource_generation=0 content_crc=0 content_frame=0 content_frame_hash=0 gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
    echo "hyperv-webkit-gpu-validate: webkit_downstream_consumer_gate_matrix validation_run_id=${VALIDATION_RUN_ID} webkit_run_id=${VALIDATION_RUN_ID} prior_dxg_run_id=none prior_fps_run_id=none fps_artifact_run_id=none backend_identity=FB_GPU_BACKEND_F_OPENGL_SUBMIT dxg_backend_opengl_submit=0 fps_backend_opengl_submit=0 backend_opengl_submit=0 backend_zero_rejected=1 display_bind_backend=none display_bind_transport=none display_bind_transport_source=none host_saw_display_bind_packet=0 wsl_presenthistory_completion_credit=0 display_bind_present_id=0 display_bind_completed_id=0 display_bind_resource_generation=0 display_bind_completion_source=missing completion_source=missing display_bind_source_authority_rejected=1 display_bind_completion_source_rejected=1 native_present_id=0 native_completed=0 native_present_ids_zero_rejected=1 lineage_rejected=1 current_run_display_bind_completion=0 current_run_native_present_identity=0 finite_480p_fps_artifact=0 rejected_reason=preflight-zero-credit gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
...
require_webkit_display_bind_dependency_skeleton_closed()
{
    require_log 'webkit_display_bind_dependency_skeleton_matrix .*evidence_path=/tmp/wlcomp-d3d12-present .*required_current_run_display_bind_completion=1 .*required_validated_shared_surface_native_d3d12_contract=1 .*required_content_progress_native_present_complete=1 .*required_backend_opengl_submit=1 .*title_only=REJECT .*chrome_only=REJECT .*cursor_only=REJECT .*render_node_only=REJECT .*dmabuf_only=REJECT .*app_loop_only=REJECT .*stale_log_rejected=PASS .*display_bind_transport_source=none .*host_saw_display_bind_packet=0 .*wsl_presenthistory_completion_credit=0 .*display_bind_present_id=0 .*display_bind_completed_id=0 .*display_bind_completion_source=missing .*completion_source=missing .*current_run_display_bind_completion=0 .*validated_shared_surface_native_d3d12_contract=0 .*content_progress_native_present_complete=0 .*backend_opengl_submit=0 .*gate=closed .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_content_credit=0 .*webkit_accel_credit=0 .*status=PASS' \
        "WebKit display-bind dependency skeleton closed"
}

require_webkit_display_bind_dependency_skeleton_open()
{
    require_log 'webkit_display_bind_dependency_skeleton_matrix .*evidence_provider=wlcomp .*evidence_path=/tmp/wlcomp-d3d12-present .*evidence_generation=[1-9][0-9]* .*evidence_run_id=[A-Za-z0-9_.:-]+ .*required_current_run_display_bind_completion=1 .*required_validated_shared_surface_native_d3d12_contract=1 .*required_content_progress_native_present_complete=1 .*required_backend_opengl_submit=1 .*title_only=REJECT .*chrome_only=REJECT .*cursor_only=REJECT .*render_node_only=REJECT .*dmabuf_only=REJECT .*app_loop_only=REJECT .*stale_log_rejected=PASS .*display_bind_backend=gpup_dxg_scanout_bind .*display_bind_transport=gpu-p-dxg-resource-scanout-bind .*display_bind_transport_source=non_wsl_linux_dxgkrnl_extension .*host_saw_display_bind_packet=1 .*wsl_presenthistory_completion_credit=0 .*display_bind_present_id=[1-9][0-9]* .*display_bind_completed_id=[1-9][0-9]* .*display_bind_resource_generation=[1-9][0-9]* .*display_bind_completion_source=display .*completion_source=display .*current_run_display_bind_completion=PASS .*validated_shared_surface_native_d3d12_contract=PASS .*content_progress_native_present_complete=PASS .*backend_opengl_submit=1 .*gate=open .*native_present_credit=[1-9][0-9]* .*opengl_submit_credit=1 .*webkit_content_credit=1 .*webkit_accel_credit=1 .*status=PASS' \
...
emit_webkit_animated_content_native_present_gate_open()
{
    ...
    local fps_display_bind_transport_source
    local fps_host_saw_display_bind_packet
    local fps_wsl_presenthistory_completion_credit
    local fps_display_bind_completion_source
    local fps_completion_source

    require_sealed_d3d12_evidence_file "${LOG}" \
        "WebKit animated content current-run provider tuple"
    content_line="$(provider_owned_display_bind_line_file "${LOG}" \
        "WebKit animated content content-progress tuple" \
        'd3d12_wayland_content_progress_matrix .*status=PASS' 0 1)"
    fps_line="$(provider_owned_display_bind_line_file "${LOG}" \
        "WebKit animated content FPS provenance tuple" \
        'd3d12_wayland_present_fps_provenance_matrix .*status=PASS' 0 0)"
    evidence_generation="$(line_log_counter "${content_line}" evidence_generation)"
    evidence_run_id="$(line_log_token "${content_line}" evidence_run_id)"
    native_credit="$(line_log_counter "${content_line}" d3d12_native_present_completion_id)"
    if [[ -z "${native_credit}" || "${native_credit}" -lt 1 ]]; then
        fail "cannot open WebKit animated content gate without provider native completion id"
    fi
    ...
    display_bind_transport_source="$(line_log_token "${content_line}" display_bind_transport_source)"
    host_saw_display_bind_packet="$(line_log_counter "${content_line}" host_saw_display_bind_packet)"
    wsl_presenthistory_completion_credit="$(line_log_counter "${content_line}" wsl_presenthistory_completion_credit)"
    ...
    fps_display_bind_transport_source="$(line_log_token "${fps_line}" display_bind_transport_source)"
    fps_host_saw_display_bind_packet="$(line_log_counter "${fps_line}" host_saw_display_bind_packet)"
    fps_wsl_presenthistory_completion_credit="$(line_log_counter "${fps_line}" wsl_presenthistory_completion_credit)"
    ...
    require_provider_owned_final_handoff_file "${LOG}" \
        "WebKit animated content final handoff" \
        "${display_bind_present_id}" "${display_bind_completed_id}" \
        "${display_bind_generation}"
    final_handoff_success=1
    final_handoff_present_id="${display_bind_present_id}"
    final_handoff_completed="${display_bind_completed_id}"
    final_handoff_generation="${display_bind_generation}"
    if [[ -z "${content_line}" || -z "${fps_line}" ||
          -z "${evidence_generation}" || -z "${evidence_run_id}" ||
          ...
          -z "${fps_display_bind_transport_source}" ||
          -z "${fps_host_saw_display_bind_packet}" ||
          -z "${fps_wsl_presenthistory_completion_credit}" ||
    ...
    if [[ "${evidence_run_id}" != "${VALIDATION_RUN_ID}" ]]; then
        fail "cannot open WebKit animated content gate with stale provider tuple run id: evidence=${evidence_run_id} expected=${VALIDATION_RUN_ID}"
    fi
    if [[ "${native_credit}" -ne "${display_bind_completed_id}" ]]; then
        fail "cannot open WebKit animated content gate with mixed native completion/display-bind ids: native=${native_credit} display_bind_completed=${display_bind_completed_id}"
    fi
    if [[ "${display_bind_backend}" != "gpup_dxg_scanout_bind" ||
          ...
          "${wsl_presenthistory_completion_credit}" -ne 0 ||
          ...
          "${fps_display_bind_transport_source}" != "non_wsl_linux_dxgkrnl_extension" ||
          "${fps_host_saw_display_bind_packet}" -ne 1 ||
          "${fps_wsl_presenthistory_completion_credit}" -ne 0 ||
...
emit_webkit_enabled_artifact_contract_matrix_open()
{
    ...
    content_line="$(provider_owned_display_bind_line_file "${LOG}" \
        "WebKit enabled artifact content-progress tuple" \
        'd3d12_wayland_content_progress_matrix .*status=PASS' 0 1)"
    evidence_generation="$(line_log_counter "${content_line}" evidence_generation)"
    evidence_run_id="$(line_log_token "${content_line}" evidence_run_id)"
    display_bind_present_id="$(line_log_counter "${content_line}" display_bind_present_id)"
    display_bind_completed_id="$(line_log_counter "${content_line}" display_bind_completed_id)"
    display_bind_generation="$(line_log_counter "${content_line}" display_bind_resource_generation)"
    display_bind_transport_source="$(line_log_token "${content_line}" display_bind_transport_source)"
    host_saw_display_bind_packet="$(line_log_counter "${content_line}" host_saw_display_bind_packet)"
    wsl_presenthistory_completion_credit="$(line_log_counter "${content_line}" wsl_presenthistory_completion_credit)"
    native_credit="$(line_log_counter "${content_line}" d3d12_native_present_completion_id)"
    ...
    content_crc="$(line_log_counter "${content_line}" d3d12_visible_content_crc)"
    content_frame="$(line_log_counter "${content_line}" d3d12_visible_content_frame)"
    content_frame_hash="$(line_log_counter "${content_line}" d3d12_visible_frame_hash)"
    content_present_id="$(line_log_counter "${content_line}" present_id)"
    content_completed="$(line_log_counter "${content_line}" completed)"
    content_generation="$(line_log_counter "${content_line}" display_bind_resource_generation)"

    if [[ -z "${native_credit}" || "${native_credit}" -lt 1 ]]; then
        fail "missing WebKit enabled artifact provider native completion id"
    fi
    ...
    if [[ "${evidence_run_id}" != "${VALIDATION_RUN_ID}" ]]; then
        fail "WebKit enabled artifact evidence run id mismatch: evidence=${evidence_run_id} expected=${VALIDATION_RUN_ID}"
    fi
...
provider_owned_display_bind_line_file()
{
    local file="$1"
    local why="$2"
    local line_pattern="${3:-}"
    local require_commit="${4:-0}"
    local require_content="${5:-0}"
    ...
    awk -v require_commit="${require_commit}" \
        -v require_content="${require_content}" \
    ...
    function nonzero_decimal(value) {
        return value ~ /^[1-9][0-9]*$/
    }
    function commit_accepted(    value) {
        value = token("d3d12_dxg_present_source_commit_successes")
        if (nonzero_decimal(value)) return 1
        value = token("d3d12_present_source_commit_successes")
        if (nonzero_decimal(value)) return 1
        value = token("d3d12_present_source_buffer_commit_successes")
        if (nonzero_decimal(value)) return 1
        value = token("dxg_present_commit_successes")
        if (nonzero_decimal(value)) return 1
        value = token("d3d12_present_source_commit_accepted")
        if (nonzero_decimal(value)) return 1
        value = token("d3d12_dxg_present_source_commit_accepted")
        if (nonzero_decimal(value)) return 1
        return 0
    }
    function content_lineage_ok(    state, visible_state, content_run,
        compositor_run, current_run, source_owned, identity_complete,
        content_present, content_completed, content_generation, visible_crc,
        visible_frame, visible_hash) {
        state = token("d3d12_content_progress_state")
        visible_state = token("d3d12_visible_content_progress")
        content_run = token("d3d12_content_progress_run_id")
        compositor_run = token("d3d12_content_progress_compositor_run_id")
        current_run = token("d3d12_content_progress_current_run_valid")
        source_owned = token("d3d12_content_progress_source_owned")
        identity_complete = token("d3d12_content_progress_identity_complete")
        content_present = token("present_id")
        content_completed = token("completed")
        content_generation = token("display_bind_resource_generation")
        visible_crc = token("d3d12_visible_content_crc")
        visible_frame = token("d3d12_visible_content_frame")
        visible_hash = token("d3d12_visible_frame_hash")
        return state == "NATIVE_PRESENT_COMPLETE" &&
            visible_state == "NATIVE_PRESENT_COMPLETE" &&
            content_run == expected_run_id &&
            compositor_run == expected_run_id &&
            current_run == "1" &&
            source_owned == "1" &&
            identity_complete == "1" &&
            nonzero_decimal(content_present) &&
            nonzero_decimal(content_completed) &&
            nonzero_decimal(content_generation) &&
            nonzero_decimal(visible_crc) &&
            nonzero_decimal(visible_frame) &&
            nonzero_decimal(visible_hash)
    }
    {
        if (line_pattern != "" && $0 !~ line_pattern)
            next
        ...
        present = token("display_bind_present_id")
        completed = token("display_bind_completed_id")
        generation = token("display_bind_resource_generation")
        bind_source = token("display_bind_completion_source")
        completion_source = token("completion_source")
        submit = token("backend_opengl_submit")
        ...
        if (backend != "gpup_dxg_scanout_bind" ||
            ...
            completion_source != "display" ||
            submit != "1" ||
            !nonzero_decimal(present) ||
            !nonzero_decimal(completed) ||
            !nonzero_decimal(generation) ||
            completed + 0 < present + 0 ||
            !nonzero_decimal(native_completion) ||
            native_completion + 0 != completed + 0)
            next
        if (require_commit && !commit_accepted())
            next
        if (require_content && !content_lineage_ok())
            next
        print
        found = 1
        exit 0
    ...
    END { exit(found ? 0 : 1) }' "${file}" ||
        fail "missing ${why} line-scoped provider-owned display-bind record in ${file}"
}

require_provider_owned_final_handoff_file()
{
    local file="$1"
    local why="$2"
    ...
    awk -v expected_run_id="${VALIDATION_RUN_ID}" \
        -v expected_present="${expected_present}" \
        -v expected_completed="${expected_completed}" \
        -v expected_generation="${expected_generation}" '
    function line_token(key,    i, n, parts) {
        for (i = 1; i <= NF; i++) {
            n = split($i, parts, "=")
            if (n == 2 && parts[1] == key)
                return parts[2]
    ...
    function block_complete() {
        return block["provider"] == "wlcomp" &&
            block["path"] == "/tmp/wlcomp-d3d12-present" &&
            block["run_id"] == expected_run_id &&
            block["generation"] == expected_generation &&
            nonzero_decimal(block["generation"]) &&
            block["host_display_commit_success"] == "1" &&
            block["success"] == "1" &&
            block["completion_correlated"] == "1" &&
            block["no_cpu_map_no_readback"] == "1" &&
            block["present_id"] == expected_present &&
            block["completed"] == expected_completed &&
            block["resource_generation"] == expected_generation
    }
    {
        provider = line_token("d3d12_final_handoff_evidence_provider")
        if (provider != "") {
            reset_block()
            block["provider"] = provider
        }
        remember("path", line_token("d3d12_final_handoff_evidence_path"))
        remember("generation",
                 line_token("d3d12_final_handoff_evidence_generation"))
        remember("run_id", line_token("d3d12_final_handoff_run_id"))
        remember("host_display_commit_success",
                 line_token("d3d12_final_handoff_host_display_commit_success"))
        remember("present_id", line_token("d3d12_final_handoff_present_id"))
        remember("completed", line_token("d3d12_final_handoff_completed"))
        remember("resource_generation",
                 line_token("d3d12_final_handoff_resource_generation"))
        remember("completion_correlated",
                 line_token("d3d12_final_handoff_completion_correlated"))
        remember("no_cpu_map_no_readback",
                 line_token("d3d12_final_handoff_no_cpu_map_no_readback"))
        remember("success", line_token("d3d12_final_handoff_success"))
        if (block_complete()) {
            found = 1
            exit 0
        }
    ...
    END { exit(found ? 0 : 1) }' "${file}" ||
        fail "missing ${why} provider-owned final-handoff record in ${file}"
}
...
require_provider_owned_display_bind_record_file()
{
    local file="$1"
    local why="$2"
    local require_commit="${3:-0}"

    provider_owned_display_bind_line_file "${file}" "${why}" "" \
        "${require_commit}" 0 >/dev/null
}
...
emit_downstream_consumer_gate_closed()
{
    ...
    local display_bind_completion_source
    local completion_source
    local backend_zero_rejected
    local native_present_ids_zero_rejected
    local display_bind_completion_source_rejected
    local display_bind_source_authority_rejected
    local lineage_rejected
    local current_run_display_bind_completion
    local current_run_native_present_identity
    local finite_480p_fps_artifact
    ...
    display_bind_transport_source="$(last_log_token "${FPS_CONTRACT_LOG}" display_bind_transport_source)"
    host_saw_display_bind_packet="$(last_log_counter "${FPS_CONTRACT_LOG}" host_saw_display_bind_packet)"
    wsl_presenthistory_completion_credit="$(last_log_counter "${FPS_CONTRACT_LOG}" wsl_presenthistory_completion_credit)"
    ...
    display_bind_completion_source="$(last_log_token "${FPS_CONTRACT_LOG}" display_bind_completion_source)"
    completion_source="$(last_log_token "${FPS_CONTRACT_LOG}" completion_source)"
    ...
    display_bind_transport_source="${display_bind_transport_source:-none}"
    host_saw_display_bind_packet="${host_saw_display_bind_packet:-0}"
    wsl_presenthistory_completion_credit="${wsl_presenthistory_completion_credit:-0}"
    ...
    display_bind_completion_source="${display_bind_completion_source:-missing}"
    completion_source="${completion_source:-${display_bind_completion_source}}"

    backend_zero_rejected=0
    if [[ "${dxg_submit}" != "1" || "${fps_submit}" != "1" ]]; then
        backend_zero_rejected=1
    fi
    native_present_ids_zero_rejected=0
    if (( display_bind_present_id == 0 ||
          display_bind_completed_id < display_bind_present_id ||
          native_present_id == 0 ||
          native_completed < native_present_id )); then
        native_present_ids_zero_rejected=1
    fi
    display_bind_completion_source_rejected=0
    if [[ "${display_bind_completion_source}" != "display" ||
          "${completion_source}" != "display" ]]; then
        display_bind_completion_source_rejected=1
    fi
    display_bind_source_authority_rejected=0
    if [[ "${display_bind_transport_source}" != "non_wsl_linux_dxgkrnl_extension" ||
          "${host_saw_display_bind_packet}" -ne 1 ||
          "${wsl_presenthistory_completion_credit}" -ne 0 ]]; then
        display_bind_source_authority_rejected=1
    fi
    lineage_rejected=0
    if [[ "${dxg_run}" != "${VALIDATION_RUN_ID}" ||
          "${fps_run}" != "${VALIDATION_RUN_ID}" ||
          "${fps_artifact_run}" != "${VALIDATION_RUN_ID}" ]]; then
        lineage_rejected=1
    fi
    current_run_display_bind_completion=0
    if (( backend_zero_rejected == 0 &&
          native_present_ids_zero_rejected == 0 &&
          display_bind_source_authority_rejected == 0 &&
          display_bind_completion_source_rejected == 0 &&
          lineage_rejected == 0 )); then
        current_run_display_bind_completion=1
    fi
    current_run_native_present_identity=0
    if (( current_run_display_bind_completion == 1 &&
          native_present_id == display_bind_present_id &&
          native_completed == display_bind_completed_id )); then
        current_run_native_present_identity=1
    fi
    finite_480p_fps_artifact=0

    echo "hyperv-webkit-gpu-validate: webkit_downstream_consumer_gate_matrix validation_run_id=${VALIDATION_RUN_ID} webkit_run_id=${VALIDATION_RUN_ID} prior_dxg_run_id=${dxg_run} prior_fps_run_id=${fps_run} fps_artifact_run_id=${fps_artifact_run} backend_identity=FB_GPU_BACKEND_F_OPENGL_SUBMIT dxg_backend_opengl_submit=${dxg_submit} fps_backend_opengl_submit=${fps_submit} backend_opengl_submit=0 backend_zero_rejected=${backend_zero_rejected} display_bind_backend=${display_bind_backend} display_bind_transport=${display_bind_transport} display_bind_transport_source=${display_bind_transport_source} host_saw_display_bind_packet=${host_saw_display_bind_packet} wsl_presenthistory_completion_credit=${wsl_presenthistory_completion_credit} display_bind_present_id=${display_bind_present_id} display_bind_completed_id=${display_bind_completed_id} display_bind_resource_generation=${display_bind_generation} display_bind_completion_source=${display_bind_completion_source} completion_source=${completion_source} display_bind_source_authority_rejected=${display_bind_source_authority_rejected} display_bind_completion_source_rejected=${display_bind_completion_source_rejected} native_present_id=${native_present_id} native_completed=${native_completed} native_present_ids_zero_rejected=${native_present_ids_zero_rejected} lineage_rejected=${lineage_rejected} current_run_display_bind_completion=${current_run_display_bind_completion} current_run_native_present_identity=${current_run_native_present_identity} finite_480p_fps_artifact=${finite_480p_fps_artifact} rejected_reason=${reason} gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
    echo "hyperv-webkit-gpu-validate: webkit_display_bind_dependency_skeleton_matrix validation_run_id=${VALIDATION_RUN_ID} webkit_run_id=${VALIDATION_RUN_ID} evidence_path=/tmp/wlcomp-d3d12-present required_current_run_display_bind_completion=1 required_validated_shared_surface_native_d3d12_contract=1 required_content_progress_native_present_complete=1 required_backend_opengl_submit=1 title_only=REJECT chrome_only=REJECT cursor_only=REJECT render_node_only=REJECT dmabuf_only=REJECT app_loop_only=REJECT stale_log_rejected=PASS display_bind_backend=${display_bind_backend} display_bind_transport=${display_bind_transport} display_bind_transport_source=${display_bind_transport_source} host_saw_display_bind_packet=${host_saw_display_bind_packet} wsl_presenthistory_completion_credit=${wsl_presenthistory_completion_credit} display_bind_present_id=${display_bind_present_id} display_bind_completed_id=${display_bind_completed_id} display_bind_resource_generation=${display_bind_generation} display_bind_completion_source=${display_bind_completion_source} completion_source=${completion_source} current_run_display_bind_completion=${current_run_display_bind_completion} validated_shared_surface_native_d3d12_contract=0 content_progress_native_present_complete=0 backend_opengl_submit=0 rejected_reason=${reason} gate=closed native_present_credit=0 opengl_submit_credit=0 webkit_content_credit=0 webkit_accel_credit=0 status=PASS" |
        tee -a "${LOG}"
}
...
require_d3d12_gpup_dda_commit_accepted_file()
{
    ...
    require_provider_owned_display_bind_record_file "${file}" "${why} D3D12 GPU-P/DDA commit accepted" 1
...
require_holistic_d3d12_display_bind_evidence_file()
{
    ...
    present_id="$(last_any_log_counter "${file}" d3d12_dxg_present_id present_id)"
    completed="$(last_any_log_counter "${file}" d3d12_dxg_present_completed completed)"
    native_completion_id="$(last_log_counter "${file}" d3d12_native_present_completion_id)"
    if (( display_bind_completed_id < display_bind_present_id )); then
    ...
    if [[ -z "${native_completion_id}" || "${native_completion_id}" -ne "${display_bind_completed_id}" ]]; then
        fail "${why} native completion id does not match display-bind completion: native=${native_completion_id:-missing} display_bind_completed=${display_bind_completed_id}"
    fi
...
webkit_shared_surface_contract_validated()
{
    ...
    require_provider_owned_display_bind_record_file "${LOG}" \
        "WebKit shared-surface contract D3D12 display-bind evidence" &&
