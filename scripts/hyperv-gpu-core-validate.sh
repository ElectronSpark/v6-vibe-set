...
require_log 'd3d12_display_bind_provider_pending_publication_matrix .*provider_submits_delta=[1-9][0-9]* .*publication_attempts_delta=[1-9][0-9]* .*dxgprocess_generation=[1-9][0-9]* .*process_adapter_generation=[1-9][0-9]* .*hmgr_index_unique_valid=1 .*device_object_ref_active=1 .*resource_object_ref_active=1 .*allocation_object_ref_active=1 .*shared_parent_snapshot_valid=1 .*opened_child_snapshot_valid=1 .*syncobject_object_ref_active=[01] .*owner_close_cancelled=0 .*publish_before_send=0 .*transport_pending_id=0 .*completion_demux_registered=0 .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*wsl_presenthistory_completion_credit=0 .*resolved_or_cancelled=1 .*refs_released=1 .*no_host_abi_cancelled=1 .*no_host_abi_refs_released=1 .*provider_no_host_abi=1 .*provider_no_sender=1 .*provider_no_completion=1 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider pending publication resolves and releases refs fail-closed until a real sender exists"
