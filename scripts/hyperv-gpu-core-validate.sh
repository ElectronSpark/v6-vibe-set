...
require_log 'dxg_scanout_bind_candidate_command_matrix .*presenthistory_cmd=34 .*redirected_flip_fence_cmd=35 .*blt_cmd=38 .*cmds_known=3 .*sender_contracts=0 .*completion_contracts=0 .*custom_host_tool=0 .*transport_present=0 .*vmbus_enum_known=1 .*linux_ioctl_contracts=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*reject_reasons=0x7f .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
    "WSL present-history candidate command IDs remain known but unusable without a scanout-bind contract"
require_log 'dxg_native_present_lane_rejection_matrix .*wsl_presenthistory_enum_only=REJECTED .*wsl_presenthistory_sender_contract=0 .*wsl_presenthistory_completion_contract=0 .*synthvid_gpa_dirty_only=REJECTED .*linux_hyperv_drm_shadow_blit_only=REJECTED .*dda_d3d12_resource_import=0 .*dda_scanout_bind=0 .*vmbus_enum_known=1 .*linux_ioctl_contracts=0 .*resource_bind_contracts=0 .*display_completion_contracts=0 .*reject_reasons=0x7f .*custom_host_tool=0 .*transport_present=0 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS' \
