...
require_log 'gpu_core_c_validator d3d12_display_bind_stale_async_completion_contract_matrix .*real_sender=0 .*completion_demux_registered=0 .*transport_pending_id=0 .*owner_close_cancel_required_after_send=1 .*owner_close_cancel_deferred=1 .*late_completion_reject_required=1 .*late_completion_reject_gate=armed .*late_completion_after_release=0 .*late_completion_rejected=not_sampled .*host_saw_display_bind_packet=0 .*display_bind_transport_source=none .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*webkit_accel_credit=0 .*status=PASS_FAILCLOSED' \
