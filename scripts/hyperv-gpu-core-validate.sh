...
require_log 'd3d12_display_bind_provider_pending_publication_matrix .*provider_submits_delta=[1-9][0-9]* .*publication_attempts_delta=[1-9][0-9]* .*publish_before_send=0 .*transport_pending_id=0 .*completion_demux_registered=0 .*resolved_or_cancelled=0 .*refs_released=0 .*provider_no_host_abi=1 .*provider_no_sender=1 .*provider_no_completion=1 .*present_id=0 completed=0 .*native_present_credit=0 .*opengl_submit_credit=0 .*status=PASS_FAILCLOSED' \
    "D3D12 display-bind provider pending publication contract remains fail-closed until a real sender exists"
