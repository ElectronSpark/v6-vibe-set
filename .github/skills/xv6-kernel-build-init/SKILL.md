...
3. For early boot failures, check linker addresses, entry path, stack setup, and first C init order.
4. For new subsystems, add init after dependencies and before first consumer.
5. Prefer current umbrella CMake behavior over xv6-tmp's older single-tree build assumptions.
