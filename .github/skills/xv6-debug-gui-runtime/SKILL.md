...
- For current xv6 MiniBrowser validation, launch WebKit with a lightweight settings profile: disable JavaScript, WebGL, WebAudio, mediasource/media-stream, and page-cache via MiniBrowser WebKitSettings options. Google's basic search form still works, while the script-heavy search-results page can otherwise starve the port after the title changes to `Google Search`.
