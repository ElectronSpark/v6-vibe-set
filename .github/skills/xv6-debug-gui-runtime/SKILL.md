...
- For current xv6 MiniBrowser validation, launch WebKit with a lightweight settings profile: keep JavaScript enabled for Google Search, but disable WebGL, WebAudio, mediasource/media-stream, and page-cache via MiniBrowser WebKitSettings options. Do not pass `default-font-size`/`minimum-font-size` as MiniBrowser argv: those WebKitSettings properties are unsigned and MiniBrowser's dynamic option generator does not expose them. Use GTK/Pango scaling such as `GDK_DPI_SCALE=1.35` for legibility instead.
