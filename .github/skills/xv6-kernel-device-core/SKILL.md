...
- Drivers: `kernel/kernel/dev/nullrand.c`, `kernel/kernel/dev/fb/`, `kernel/kernel/dev/evdev.c`, `kernel/kernel/tty/tty_dev.c`.
- Per-open dispatch: `kernel/kernel/vfs/file.c`; `cdev.open_file` may install custom file operations and private state.
...
3. For readiness issues, make `.poll` report the current level and identify whether waiters attach to the cdev or the per-open file. evdev and PTY install file operations, so notifying only the cdev list can miss their waiters. Preserve file references while notifying and avoid driver-lock / poll-lock inversion; route wait delivery to [event-wait](../xv6-kernel-event-wait/SKILL.md).
