...
- Plasma/Konsole responsiveness: the dominant kernel-side cost class is
  ...
  corruption.
  A gated implementation now exists for item (1), default off:
  `x86_cr3_noflush=1` is honored only when existing PCID support is active
  (`x86_pcid=1`, boot log `vm_asid_init: max ASID = <nonzero>`). The return
  path skips redundant TRAPFRAME PTE rewrites, reports whether the per-CPU
  slot changed, and `userret` performs a targeted `invlpg` after loading the
  user CR3 so the invalidation targets the user PCID. Kernel build and static
  whitespace gates passed after the implementation. A3 validation accepted
  the opt-in gate with caveat: in nographic PCID-only control, `syscalltlb
  2000` reported TLB amplification of `1678/2601/1153/2647ns` for
  `64/256/512/1024` pages; with `x86_pcid=1 x86_cr3_noflush=1`, those became
  `0/0/1213/0ns` with `vm_asid_init: max ASID = 4095`, proving the hidden
  per-syscall TLB refill tax is mostly removed. The gate-on KDE
  desktop-interaction pass
  (`20260702T020333Z-a3-noflush-gate-on-pass`) completed with `status_code=0`,
  `first_visible_ms=19760`, and `konsole_wait_ms=3643`; the matched PCID-only
  control (`20260702T020533Z-a3-noflush-gate-off-pcid-control-pass`) also
  completed with `status_code=0`, `first_visible_ms=19504`, and
  `konsole_wait_ms=3998`. `clonetest` and `cowtest` passed in both nographic
  boots. The standalone `forktest` returned `rc=1` with `fork claimed to work
  N times!` in both control and gate-on boots; this is the old
  process-table-exhaustion expectation, not evidence of a noflush-specific
  fork return-value regression. No crash/RCU/coredump markers were found in
  either KDE archive. The default must remain unchanged until broader
  regression exposure, but the opt-in reducer gate passed this session.
  2026-07-02 status re-check via nographic VM A/B (`syscalltlb 2000`, booted
  cmdline and ASID line verified in both runs): default boot
  (`max ASID = 0`) still shows the bottleneck — `getpid` `3.7-6.0us`, TLB
  amplification `386/0/1662/2569ns` for `64/256/512/1024` pages; gate-on
  boot (`x86_pcid=1 x86_cr3_noflush=1`, `max ASID = 4095`) removes the
  amplification in three of four buckets (`19/0/5789/0ns`; the 512-page
  sample is a noisy outlier whose warm-sweep baseline also doubled) while
  fixed `getpid` cost remains `~3.2-3.9us` as expected because MSR writes,
  trapframe copies, CR0.TS, and cpumask atomics are untouched by the gate.
  Status: fix works opt-in, default unchanged, promotion still blocked on
  broader regression exposure.
  2026-07-02 freeze-report triage: a user-reported VM freeze while Chromium
  loaded the YouTube main page in the KDE session was reproduced and
  A/B-cleared against this fix. The frozen boots did NOT carry
  `x86_pcid=1`/`x86_cr3_noflush=1` (gate inert; the only default-path delta
  is skipping identical TRAPFRAME PTE rewrites), and a control run with the
  fix stashed and the pre-fix kernel rebuilt froze identically at the same
  phase. Evidence archives:
  `build-x86_64/yt-mainpage-freeze-repro/20260702T042326Z-fixed-kernel-gate-off/`
  and
  `build-x86_64/yt-mainpage-freeze-repro/20260702T044322Z-prefix-kernel-control-same-freeze/`.
  Freeze shape on both kernels: Chromium fully launches (browser, GPU,
  renderer with `VideoFrameCompo`/`Media`/`VizCompositorTh` threads), then
  userspace stops making visible progress while the kernel stays alive —
  serial TTY echoes input but bash never executes, no panic, no virtio
  timeout. The in-kernel thread dump captured in the fixed-kernel run shows
  the decisive signature: roughly 50 user threads in `R` (runnable) state
  (plasmashell, kwin_wayland, WaylandEventThr, QDBusConnection, dbus,
  portals, bash) while 3-4 of 6 CPUs sit halted-idle, and repeated GDB
  samples show the busy CPUs in `__do_scheduler_wakeup` -> `rq_trylock_two`
  retry loops, `__sched_timer` lock contention in `timer_tick`/
  `sched_timer_set`, and wakeup IPI sends. Interpretation: a pre-existing
  runnable-threads-not-running scheduler/wakeup starvation lane under
  Chromium-scale thread counts (~200+ threads), not a TLB/CR3 regression.
  Reproduction recipe (deterministic within ~2-6 min of Chromium launch):
  boot the KDE image with the standard flags, then from the serial console
  run `XDG_RUNTIME_DIR=/dev/shm/xdg-runtime-root WAYLAND_DISPLAY=wayland-0
  /bin/wayland-chromium https://www.youtube.com/`. Next steps for this new
  lane: route through `xv6-kernel-freeze-triage` and
  `xv6-kernel-process-scheduler`; capture rq lock owners, per-CPU
  `NEEDS_RESCHED`, wake-list depth, and idle-CPU wakeup/IPI acceptance at
  freeze time before patching `rq_trylock_two` backoff or wakeup IPI
  behavior. The control run also recorded Chromium children being
  SIGTERM-killed (`Chrome_ChildIOT`, `ServiceWorker t`,
  `reason=pending-delivery`) before the freeze completed; treat that as a
  secondary clue, not yet root cause evidence.
- Chromium video frame dropping (`presentedFPS=27.2` vs `decodedFPS=60.6`):
  ...
  Follow-up ordered-pageflip promotion validation on 2026-07-02 did NOT reach
  the required second clean desktop-interaction pass. The flag was present in
  the booted cmdline for both archived attempts, but
  `20260702T013105Z-desktop-interaction-ordered-pageflip-visible-timeout-fail`
  ended with `status_code=8` / desktop visible timeout, and
  `20260702T013252Z-desktop-interaction-ordered-pageflip-kwin-gp-fail` ended
  with `status_code=3` after `kwin_wayland` #GP. Neither failure matched the
  known `rcu_head_cache` double-free signature; read-only archive review
  classified them as known non-RCU GUI flake classes rather than enough
  evidence to blame the ordered-pageflip flag. Promotion remains blocked:
  do not flip the default without a fresh clean ordered-pageflip pass, a
  matched default-off control in the same session, and the later
  Chromium-video FPS A/B after renderer admission is fixed.
  B3 retry completed the missing clean ordered-pageflip desktop pass and a
  matched default-off control in the same session. Ordered archive
  `20260702T021106Z-b3-desktop-interaction-ordered-pageflip-pass` passed with
  `status_code=0`, `first_visible_ms=22190`, `first_nonzero_ms=12859`, and
  `konsole_wait_ms=4459`, with `virtio_gpu_ordered_page_flip=1` present in
  the booted cmdline. Matched default-off archive
  `20260702T021301Z-b3-desktop-interaction-default-off-control-pass` passed
  with `status_code=0`, `first_visible_ms=16110`, `first_nonzero_ms=10985`,
  and `konsole_wait_ms=4086`, with the ordered flag absent. No RCU, KWin #GP,
  visible-timeout, panic, coredump, or assertion markers appeared in either
  run. Decision: `virtio_gpu_ordered_page_flip=1` has enough desktop-smoke
  evidence to stay ready for the decisive Chromium-video FPS A/B, but do NOT
  flip the default until renderer admission is fixed and that Chromium-video
  A/B proves the presented-FPS gain.
...
2. KDE/Chromium GUI ABI and performance:
   ...
   - 2026-07-02 pasted pactl fault update: a `pactl` audio probe faulted at
     `cr2=0x7fffff7be0b8` with `rsp=0x7fffff7be0b8`, just below the mapped
     stack VMA `[0x7fffff7bf000-0x7ffffffbf000)`. That VMA is already the
     8 MiB `MAXUSTACK`, so this is not a simple missed growdown case. The
     live register dump pointed at Pulse strings
     `../src/pulsecore/thread-posix.c` and
     `pthread_key_delete(t->key) == 0`; `libpulsecommon-16.1.so`
     `file_off=0x305c0` maps to `pa_log_levelv_meta`, with the repeated
     frames running through `pa_log_level_meta`, `pa_tls_set`, and
     `pa_thread_self`. KDE/Xwayland/KWin/plasmashell kept running and page
     flips continued, so treat this as the existing nonfatal Pulse/pactl
     audio-startup failure, not a virtio page-flip or CR3 no-flush
     regression. The session-ready harness now has an opt-in
     `KDE_SMOKE_TOLERATE_OPTIONAL_PACTL_READY_CRASH=1` carveout that ignores
     only `pactl` ready-phase crashes and still fails KWin/plasmashell/Xwayland
     or kernel crashes. Do not use `kde_audio=0` for accepted desktop,
     responsiveness, Chromium-video, or promotion proofs: audio is part of the
     guard surface. The next reducer must keep audio enabled and isolate
     pactl/PipeWire startup with `audio_unix_ipc_trace` and stack evidence; do
     not raise the VM stack cap or change page-flip defaults based on this log
     alone.
